...
it('refuses an unsigned webhook with 400', function (): void {
    ...
    postJson('/webhooks/viva', WebhookScenario::gatewaySuccess('evt_unsigned'))
        ->assertStatus(400)
...
it('refuses a wrongly signed webhook with 400', function (): void {
    ...
    $payload = WebhookScenario::gatewaySuccess('evt_wrong_secret');

    postJson('/webhooks/viva', $payload, WebhookScenario::verifiedHeaders($payload, 'whsec_someone_elses_secret'))
        ->assertStatus(400);
})->group('fast');
...
it('refuses a valid signature over an old payload, because that is a replay', function (): void {
    ...
    $payload = WebhookScenario::gatewaySuccess('evt_stale');
    ...
    postJson('/webhooks/viva', $payload, [
...
it('records an unverified webhook with its source IP in the log', function (): void {
    ...
    postJson('/webhooks/viva', WebhookScenario::gatewaySuccess('evt_logged'))->assertStatus(400);
...
it('gives every unverified request its own row rather than deduplicating them', function (): void {
    ...
    for ($i = 0; $i < 3; $i++) {
        postJson('/webhooks/viva', ['nothing' => 'useful'])->assertStatus(400);
    }
...
it('keeps the payload out of anything serialised', function (): void {
    ...
    postJson('/webhooks/viva', $p = WebhookScenario::gatewaySuccess('evt_hidden'), WebhookScenario::verifiedHeaders($p))->assertOk();
    ...
    // The sentinel is a value that lives **only** in the payload. The order
    // code will not do: Viva sends no event id, so the controller derives one
    // from the order code — which then legitimately appears in `event_id`, and
    // asserting its absence would fail on a row that is leaking nothing.
    expect($event->toArray())->not->toHaveKey('payload')
        ->and((string) json_encode($event))->not->toContain('evt_hidden');
})->group('fast');
