...
| `pdf-chromium` | The `chromium` group — the e-ticket PDF **asserted** against Browsershot's output (ENV-20, BKG-13.1). Excluded from `composer test`, because a developer with no browser must not get a false green. It is not the only place a PDF is *rendered*, and the sentence here used to say it was: `GenerateETicketOnConfirmation` is a queued listener and the test queue is `sync`, so every booking that confirms anywhere in the suite prints a ticket, and the invoice and charter-agreement tests call their renderer directly. Every job that runs the suite therefore installs Puppeteer and is pointed at the runner's Chrome — without that, thirty-eight tests in files that have nothing to do with PDFs fail with `Cannot find module 'puppeteer'`, which is what red-lit every run between 6 and 14 September 2026. |
...
| `plugin-lint` | phpcs over the WordPress plugin — the WordPress coding standard and, just as importantly, **PHP 8.1 compatibility** (WPP-11) — plus the plugin's own unit tests. It is the one job that does not run on 8.4, because the plugin runs on operator hosting and a developer with 8.4 habits will otherwise ship something that fatals there. |
...
widget build with #106, the end-to-end run with #111 and the plugin standard with the first M4 issue —
each on the day it stopped being a stub and started taking real time. `node-checks` is gone; nothing is
left in it. **Nothing was dropped**: every command that ran before
