...
| `plugin-lint` | phpcs over the WordPress plugin: the WordPress coding standard and, just as importantly, **PHP 8.1 compatibility** (WPP-11). It is the one job that does not run on 8.4, because the plugin runs on operator hosting and a developer with 8.4 habits will otherwise ship something that fatals there. |
