...
- **TEN-1 (FIXED)** **Tenant (Operator)** — name, slug, legal name, ΑΦΜ, ΔΟΥ, address, timezone, default locale, currency (EUR), plan, `trial_ends_at`, `custom_domain`, `hosted_page_enabled`.
...
- **PRC-19** Voucher surplus and restoration (per [ADR-0017](adr/0017-voucher-remainder-and-refund-restoration.md), Options A and D). **Voucher value stays voucher value and cash stays cash; neither is ever converted into the other.**
  ...
  4. **Ledger** — every movement in either direction is a row in `voucher_redemptions` (voucher, booking, amount, direction, reason, timestamp). `vouchers.remaining_cents` is a denormalised value that MUST always be reconstructible from the ledger; a reconciliation test asserts it. Cancellation writes a reversal row and never deletes a redemption row.
...
- **PRC-27** Balance due date and unpaid-balance handling (per [ADR-0018](adr/0018-balance-due-policy.md), Option A):
  1. **Due date** — `tenants.balance_due_days_before_departure`, default **14**, with an optional per-RatePlan override (`rate_plans.balance_due_days_before_departure`) taking precedence. The due instant is `starts_at_utc` minus N days, floored to **09:00 in the tenant timezone**.
...
- **CXL-3** Refund computation for a guest-initiated cancellation at instant T:
  ...
  3. `refund_cents = round_half_up(paid_cents * refund_percent / 100)`. The base is the amount actually paid in cash, not the booking total.
...
- **CXL-8** A voucher issued for a force-majeure cancellation expires `force_majeure_voucher_months` (default 18) after issue, from the policy snapshot.
...
| CXL-3 | Tier selection and refund base | Largest qualifying tier; base is cash actually paid. |
...
- **BKG-3 (FIXED)** Every booking carries a human reference (format `KAI-7F3K2`). Format and collision strategy (per [ADR-0007](adr/0007-booking-reference-format.md), Option A):
  1. **Shape** — `KAI-XXXXX`: a configurable brand prefix (config value, default `KAI`) plus a hyphen plus **5 characters**, stored as `char(9)`.
  2. **Alphabet** — 31 unambiguous symbols: the digits and uppercase letters minus `0`, `O`, `I`, `1`, `L` and `U`. That is ~28.6 million combinations. Stored uppercase; compared case-insensitively; guest input normalised by uppercasing and stripping non-alphanumerics before lookup.
...
- **BKG-7** Required fields at draft creation: product, date, start time (per-vessel with flexible start), pax breakdown by age band, extras with quantities, lead guest name, email and phone, locale, optional voucher code, optional special requests, and explicit consent to the operator terms and privacy policy with a stored timestamp.
...
- **BKG-32** A manual booking may exceed `min_lead_time_hours` and `max_advance_days` restrictions but MUST NOT exceed capacity or the legal `capacity_max` (AVL-25). Capacity override requires an explicit confirmation and is logged.
...
- **PAY-4** Credential storage shape and the deposit/balance model (per [ADR-0004](adr/0004-payment-credentials-and-deposit-model.md), Options A and D):
  1. **Credential storage** — a `payment_gateway_accounts` table, one row per (`tenant_id`, `gateway`, `mode`) where `mode` is `live` | `sandbox`, holding an `encrypted`-cast credential JSON blob plus `is_default`, `verified_at` and `last_verified_error`. This is the §3 mandate (PAY-3) with no new infrastructure; there is no external secret store and no per-tenant key separation in MVP. Residual risk is handled by encrypting backups with a key separate from `APP_KEY`, keeping `APP_KEY` out of the backup set, and never logging credentials (MYD-15, SEC-9).
...
- **WGT-5 (FIXED)** Mounts: `booking` (single product), `list` (trip grid with a from-price and category tabs), `calendar` (availability only), `enquiry`.
- **WGT-6 (FIXED)** Attributes: `data-key`, `data-product`, `data-locale`, `data-theme`, `data-category`.
- **WGT-7** Additional attributes: `data-mount` (one of the four mounts, defaulting to `booking` when `data-product` is present and `list` otherwise), `data-analytics` (on by default), and `data-target` (a CSS selector for the mount node, defaulting to the script tag position).
...
- **WGT-18** The booking mount walks: date selection, party composition by age band, extras, contact details and consent, review with the full price breakdown, then redirect to the gateway. Back navigation between steps never loses entered data.
...
- **WGT-20** After the gateway redirect returns, the confirmation state is driven by the booking status endpoint, polling for up to 60 seconds while the webhook lands, then falling back to a message promising an email once payment is confirmed.
...
- **HOS-6** A hosted page for a tenant with `hosted_page_enabled = false` returns 404.
...
- **WPP-4 (FIXED)** Shortcodes: `[kaiki_booking product="uuid"]`, `[kaiki_list category="shared"]`, `[kaiki_calendar product="..."]`, `[kaiki_enquiry]`.
