...
- **OPS-26** Nothing on the statistics page requires tracking a visitor. Every figure is derived from rows the product already writes, so the page needs no cookie, no consent question and no third party (GDR-12). Counting **visits** rather than bookings is a separate decision with its own storage and its own ADR.
---
