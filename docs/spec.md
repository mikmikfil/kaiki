...
- **BKG-20 (FIXED)** On the day of departure, crew check in guests through a QR scan page in Filament that works on a phone; the manifest can be exported at any time.
  ...
  - *Amended 2026-09-14 by the product owner:* **check-in itself is optional too**, on the same screen and with the same trail (`tenants.check_in_enabled`, default on, audited). Off, neither boarding page opens — no scan box and no passenger list with a tap per name — and tickets carry no QR, because `Tenant::usesQrCheckIn()` reads this switch first and the two cannot disagree. Everything that is a record rather than a gesture is unchanged: the manifest still exports (BKG-20), the sweep still completes departures (BKG-21), and a booking that is never boarded still reaches `completed`. The sentence above that read *"check-in itself is unchanged: only the scan is optional"* is superseded by this one.
...
- **OPS-23** A **statistics page** in the operator panel answers *how did we do* over a period the operator picks — presets plus their own two dates, carried in the URL so a report can be sent to somebody. Added 2026-09-14 at the product owner's request; the spec carried no reporting requirement before it.
- **OPS-24** Every block on that page states its **basis**, and there are exactly three, because one period for everything produces figures that quietly disagree: **revenue** is money received (`payments.paid_at`, succeeded, minus refunds — the same definition OPS-2 fixed for the dashboard, over a different window); **bookings and passengers** are sales made (`bookings.created_at`, committed statuses only); **occupancy** is seats sold over seats offered on departures that have **already sailed**. A departure that has not left yet is excluded: counting it makes every forward-looking period read as empty.
- **OPS-25** The page shows, at minimum: revenue, bookings, passengers and average booking against the previous equal period; revenue over time; bookings, passengers and revenue per trip and per boat; occupancy overall, per month and per trip; the emptiest sailings; the channel each booking came through (`bookings.source`) with its utm tags and referrer host; and cancellations with their rate. Test bookings are excluded throughout (SAA-12).
- **OPS-26** Nothing on the statistics page requires tracking a visitor. Every figure is derived from rows the product already writes, so the page needs no cookie, no consent question and no third party (GDR-12).
- **OPS-27** **Visits and the funnel are counted first-party and cookieless** ([ADR-0032](adr/0032-first-party-visit-counting.md), decided 2026-09-14). An `analytics_daily` **rollup** holds one row per tenant per local day per metric: no visitor id, no session, no IP address, no user agent and no row per person. Hosted page views are counted on the server; the widget's eight WGT-11 events are beaconed to `POST /api/v1/events`, which accepts only those names and answers `204` whatever happens. Because nothing is written to a visitor's browser, GDR-12 holds and no consent banner is required.
- **OPS-28** The funnel is presented as **counts per step and the ratio between consecutive steps**, and never as a per-person conversion rate — cookieless means no visitor is followed from one step to the next, and the screen says so. Adding an identifier to get a true conversion rate is a new decision, not an extension of ADR-0032.
---
