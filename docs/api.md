...
| 12 | **OPEN — the exact Viva webhook verification mechanism.** Viva's scheme has changed across API generations. | Verification-key handshake **plus** a mandatory server-side re-fetch of the transaction; the webhook body is never trusted for amounts. The re-fetch stands whatever the handshake turns out to be. | A factual question about a third party, not a fork. It is answered by reading Viva's current documentation, not by choosing. | At implementation time in M2 — confirm against live official docs, never from memory. **2026-09-16: still open, and cheaper to answer.** The handshake is implemented as Viva documents it today — a GET on the operator's own address answered with the account's verification key — and the key is fetched with the OAuth2 token rather than asked of the operator. Both moving parts are configuration: `kaiki.payments.viva.webhook_key_path` for where the key is read from, and the answer prints `Key` and `key` because Viva's samples and their webhook guide disagree on the spelling. A correction is an `.env` line. |
