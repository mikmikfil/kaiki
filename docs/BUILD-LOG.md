...
| Quality gate | Pint · PHPStan level 6 + Larastan · Pest (2880 in the `fast` group, all passing, 8989 assertions, run locally on 2026-09-21; the `mysql` and `chromium` groups are skipped with a stated reason where their environment is absent). **The schema snapshot is no longer a standing failure** — CI regenerated it on run 35330810788 and no migration has landed since `efafa2c`. · **Vitest (76), the widget's four build gates and the 17-spec Playwright run**  · **the `chromium` PDF group, which until #88 no CI job ran** · **AVL-44 overselling gate, live at last** · **cross-tenant isolation gate** · **ENV-8 JSON-path gate** · **phpcs over the WordPress plugin, at PHP 8.1** · EL/EN parity · OpenAPI drift · coverage of `app/Domain` · dependency audits · schema drift — **green locally; see the CI row below** |
| Deployment | Deliberately last (#13, #14 moved to `M8 — Launch & deployment`) |
| **CI** | **Running again since 2026-09-14**, after being blocked from 2026-09-06. GitHub Actions had refused to start any job on this repository — every job on run 34028822331 failed in two seconds with no steps and no log — from an account-level setting rather than anything in this repository. It cleared on the 14th, and the first thing through was the `migrate-from-zero` artifact committed unchanged as `f9d10ff`. It has run since: `d59aa49` carries run [35218202602](https://github.com/mikmikfil/kaiki/actions/runs/35218202602) (2026-09-17) and `efafa2c` run [35330810788](https://github.com/mikmikfil/kaiki/actions/runs/35330810788) (2026-09-18), each refreshing the ENV-10 MySQL schema snapshot — the thing only CI can do, because CI is the only place with a MySQL 8 connection. **The snapshot is current: no migration has landed since `efafa2c`.** Two things this row used to say and should not say again: nothing is enforcing a merge block (`main` carries no branch protection and no rulesets, checked against the API — the caution about unmigrated branches was sound, the *mechanism* was imagined), and the snapshot is no longer an unregenerable failure. |
...
## Payments confirm themselves when the webhook does not (docs/api.md item 12)

An afternoon of testing against a real Viva demo account, and every way a webhook can fail happened in it. The verification endpoint answered 502, because the key was being fetched from the `api` host with a bearer token and it lives on the `checkout` host behind Basic auth. Once that was fixed the address registered — against **«Order Updated»**, an event that never fires on a payment, so Viva correctly never called. Twice the money moved, the guest came back to «Στη σελίδα πληρωμής», and the booking sat unconfirmed. **Taken, and nobody knows** is the worst state this system has.

`docs/api.md` item 12 has asked for the answer since M0: *"a mandatory server-side re-fetch of the transaction; the webhook body is never trusted for amounts. The re-fetch stands whatever the handshake turns out to be."*

**`ProvidesTransactionStatus`** is that re-fetch — a gateway that can be *asked*, kept off `PaymentGateway` because ADR-0004 fixes that at four methods, exactly as `ProvidesWebhookVerificationKey` is. Viva's implementation is `GET {checkout host}/api/transactions?ordercode=…` with the Merchant ID / API key pair, the same authentication split as the key. `StatusId` is a single letter: `F` is paid, `E`, `X` and `C` are over and unpaid, everything else — `A` above all, a guest on the 3-D Secure step — is not settled. Where an order has several attempts, a success wins: what matters is whether the money is there now.

**`ReconcilePendingPayments`** walks payments still `pending` or `processing` after five minutes and under three days old, asks, and routes the answer through `ConfirmFromWebhook` — the identical path a webhook takes, so the transition, events and notifications are the same and a webhook arriving afterwards is the no-op it always was (AVL-47). `php artisan payments:reconcile`, scheduled every five minutes, `withoutOverlapping`.

**The two refusals are the design.** Silence never fails a payment: an unreachable gateway, a dead credential, an unknown order all mean *no answer*, and only an explicit "over and unpaid" fails one — a reconciler that read an outage as failure would cancel bookings across the platform. And nothing is confirmed without checking the amount: a figure that is not the one owed logs `payments.reconcile_amount_mismatch` and waits for a person, because confirming against a number nobody compared is trusting a webhook body one step later. Euros are read into cents through a string, since `(int) (110.00 * 100)` is not reliably 11000.

**Verification.** Run against the live demo account, it confirmed both real payments — `KAI-H28XF` and `KAI-4FFYX`, 110,00 € each — including the one made before the webhook was registered at all, which no webhook could ever have recovered: *checked 3, confirmed 2, failed 0, no answer 1*, the third being a fake-gateway row it correctly left alone. Eight tests (`ReconcilePendingPaymentsTest`): paid confirms; unsettled waits; `E` fails; a 503 changes nothing; a wrong amount refuses and logs; a retried card takes the success; a payment inside the grace period is not even asked about (`Http::assertNothingSent`); and 110.00 comes back as exactly 11000 cents.

**Also in this pass:** `VivaCredentialVerifier`, and it was not cosmetic. Nothing could write `verified_at`, `usableForRealCall()` requires it, so `GatewayResolver` treated every credential set as unusable — a sandbox tenant fell through to the fake checkout and a live one would have been told the payment could not start. It checks both credential pairs, because Viva takes two and they sit next to each other on the same dashboard page. `TRUSTED_PROXIES` via `config/trustedproxy.php`: behind the tunnel the app saw the proxy's plain HTTP hop and rendered an HTTPS page whose form posted to `http`, which the browser blocked outright — and `env()` in `bootstrap/app.php` runs before `.env` is loaded, so the first fix silently did nothing. Deployment behind Caddy would have hit the same wall. The `pay()` POST now sets the guest's locale, so a refusal is answered in the language the page was filled in.

**Not built, and named:** nothing yet tells an operator that a booking was confirmed by reconciliation rather than by webhook, and it is worth surfacing — a webhook that has silently stopped delivering looks identical to one that works, from the panel. Viva still has not called this installation once, with the right event registered and active; its delivery log is the next place to look.

---

## Viva, asked for what its own plugin asks for — and a webhook address per operator

The product owner set up Viva's WooCommerce plugin with a client id and a client secret and came back with a fair question: *why does Kaiki want more?*

**It shouldn't ask for the verification key, and now it doesn't — but the first attempt got the reason wrong.** The key was believed readable with the OAuth2 token the client credentials mint. It is not. Probing every plausible address against the demo account settled it: `{checkout host}/api/messages/config/token` answers **401** to a bearer token and 401 anonymous — so the path is real and wants Basic auth over **Merchant ID and API key** — and the `api.` host answers **404** at every spelling, so there is no OAuth2 equivalent. Viva splits its APIs across two authentication schemes and the webhook key is on the other side of the split. So `credentialFields()` for Viva is four values now, not two, and `VivaSmartCheckoutGateway::webhookVerificationKey()` fetches the key with the Basic pair, stores it on the credential and hands it to the endpoint that has to print it. The operator still never makes that call by hand, which was the point — and all four values sit on one dashboard page (Settings → API Access), so it is copying rather than computing. `IntegrationProvider::fetchesWebhookSecret()` is the new distinction, and `requiresWebhookSecretFromOperator()` is what the form asks: Viva no longer refuses a complete credential set for want of a value an operator would have had to make an API call to find. Postmark still asks, because its key is issued once at creation and cannot be read back. **A key pasted by hand is left alone** — an operator who has one is ahead of us. The path is `kaiki.payments.viva.webhook_key_path`, configuration rather than a constant, because `docs/api.md` item 12 still records the exact mechanism as an open question against Viva's live documentation; a wrong path is then an `.env` line rather than a release.

**The source code stays a field.** Viva requires `sourceCode` on every payment order — it names which of the merchant's payment sources takes the money — and the WooCommerce plugin asks for it too. So the form is client id, client secret, source code: the same three things, and no fourth.

**Then the webhook itself, which could not have worked.** Viva verifies a webhook address by **GET-ing it** and expecting the account's verification key in the answer. `/webhooks/{provider}` was POST-only, so saving the address in Viva's dashboard would have failed — and the deeper problem is that the verification call carries nothing identifying: no source code, no merchant id, no signature. One shared address cannot answer it, because it cannot know whose key to print.

So the operator is in the path: `/webhooks/viva/{token}`, GET to verify and POST to deliver, with a 40-character token minted on the first save and never rotated (the address is pasted into somebody else's dashboard; rotating it would break it silently). **Not the source code**, which was the other candidate and is already indexed: four digits are walkable, and whoever reads a verification key off an answer can forge that operator's webhooks. The panel prints each credential's address beside the Success and Failure URLs it already showed.

The second thing the address buys is that **the sender stops choosing its own verifier**. On the shared route the credential — and therefore the key compared against — came from `EventData.SourceCode`, a field in a body written by whoever is posting. The old route still works unchanged for anything already pointed at it.

**Verification.** Nine new tests (`WebhookPerTenantAddressTest`): the key is printed to whoever holds the address; an address nobody owns is a bare 404; an absent key is fetched from the gateway and stored; a gateway refusal is 502 rather than a plausible-looking lie; a delivery on the address verifies against that operator; a wrong key on the right address is 400; a payload naming somebody else's source is still matched to the address's operator; the token is minted once and survives later saves; and a Viva credential set with no webhook key is accepted. With Payments, Webhook, Integration and Checkout: 183 passed before the new file, 192 with it.

**Not built, and named:** nothing re-checks that the fetched key is still current if an operator regenerates it in Viva — the next failed verification is how that would surface. The `docs/api.md` item 12 question is still open; this change makes it cheaper to answer, not answered.
